<template>
  <v-container>
    ...
    <v-form v-model="valid" ref="form">
      <v-row>
        <v-col cols="12" md="4">
          <v-text-field @keydown.enter="namaPemasok" id="nama-pemasok" v-model="form.namaPemasok" :rules="namaPemasokRulers"
            label="Nama Pemasok" required></v-text-field>
        </v-col>
        <v-col cols="12" md="4">
        ...
        <v-col cols="12" md="4">
          <v-text-field @keydown.enter="kotaPemasok" id="kota" v-model="form.kotaPemasok" :rules="kotaPemasokRulers"
            label="Kota Pemasok" required></v-text-field>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12" md="4">
    ...
    <div>
      ...
      <v-data-table :headers="headpemasok" :items="dtpemasok" :search="cari" :items-per-page="5" class="elevation-1">
...
export default {
  data() {
    return {
      ...
      headpemasok: [
        { text: "No", value: "id" },
        { text: "Nama Pemasok", value: "nama_pemasok" },
        { text: "Kode Pemasok", value: "kode_pemasok" },
        { text: "No. Telp", value: "no_telp" },
        { text: "Kota Pemasok", value: "kota_pemasok" },
      ...
      kotaPemasokRulers: [
        (v) => !!v || 'Kota Pemasok tidak boleh kosong!',
      ],
      alamatRulers: [
  ...
  methods: {
    hapus(item) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.dialog = true
          this.$axios
          .delete(this.apiurl + `/delete-pemasok/${item.id}`)
          .then(() => {
            this.loadPemasoks()
            Swal.fire({
              position: 'top-end',
    ...
    update(form) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.dialog = true
          this.$axios
            .put(this.apiurl + '/update-pemasok/' + form.id, {
              'namaPemasok': this.form.namaPemasok,
              'noTelp': this.form.noTelp,
              'kotaPemasok': this.form.kotaPemasok,
    ...
    edit(item){
      ...
      this.form.namaPemasok = this.dtedit.nama_pemasok
      this.form.noTelp = this.dtedit.no_telp
      this.form.kotaPemasok = this.dtedit.kota_pemasok
      this.form.email = this.dtedit.email
      this.form.alamat = this.dtedit.alamat
    },
    save() {
      if (this.form.namaPemasok == null || this.form.noTelp == null || this.form.kotaPemasok == null || this.form.email == null || this.form.alamat == null) {
        Swal.fire({
          icon: 'error',
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            this.dialog = true
            this.$axios
            .post(this.apiurl + '/insert-pemasok', this.form)
            .then(() => {
              this.loadPemasoks()
              this.$refs.form.reset()
              Swal.fire({
    ...
    loadPemasoks(){
      this.$axios.get(this.apiurl + '/load-pemasok')
      .then((ress)=>{
        this.dtpemasok = ress.data.data
      })
      .catch(() => {
        console.log(err);
      })
    ...
    baru(){
      this.$refs.form.reset()
      document.getElementById('nama-pemasok').focus()
      this.updateSubmit = false
    },
    batal(){
      this.$refs.form.reset()
